...
// ============================================================================
// Tab: Conexión (Super Admin only)
// ============================================================================

function ConexionTab({ active }: { active: boolean }) {
  const { isSuperAdmin } = usePermission();
  const { allowed } = useHybridPageGate();
  ...
  useEffect(() => {
    if (!active || !isSuperAdmin || !allowed) return;
    void load();
  }, [active, isSuperAdmin, allowed, load]);
    ...
    <div className="space-y-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Solo lectura. El token nunca se muestra en pantalla.
          </p>
        </div>
        <Button
          type="button"
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        {isSuperAdmin ? (
          <TabsContent value="conexion">
            <ConexionTab active={activeTab === 'conexion'} />
          </TabsContent>
        ) : null}
