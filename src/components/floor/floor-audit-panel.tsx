...
import { ClipboardList, Loader2, UserRound, Calendar } from 'lucide-react';
    ...
    <ul className="mt-1 space-y-0.5 text-[12px] text-muted-foreground">
      {order.items.map((i) => (
        <li key={i.id}>
          ...
          {i.name}
          {i.station === 'BAR' && <span className="ml-1 text-amber-500">barra</span>}
          {i.station === 'KITCHEN' && <span className="ml-1 text-sky-400">cocina</span>}
        </li>
      ))}
    </ul>
  );
...
export function FloorAuditPanel() {
  ...
  const createdById = hostFilter !== 'all' ? Number(hostFilter) : undefined;
        ...
        <>
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
            <div className="rounded-2xl border border-border/60 bg-card/70 px-4 py-4">
              ...
              <p className="mt-1 text-3xl font-semibold tabular-nums">{data.summary.orders}</p>
            </div>
            <div className="rounded-2xl border border-border/60 bg-card/70 px-4 py-4">
              <p className="text-[11px] uppercase tracking-wider text-muted-foreground">Total USD</p>
            ...
            <div className="rounded-2xl border border-border/60 bg-card/70 px-4 py-4">
              ...
              <p className="mt-1 text-3xl font-semibold tabular-nums">{data.summary.byUser.length}</p>
            </div>
          </div>
                ...
                {data.summary.byUser.map((row) => (
                  ...
                  >
                    ...
                    <div className="text-right tabular-nums">
                      ...
                      <p className="text-xs text-muted-foreground">{formatUsdAmount(row.totalUsd)}</p>
                    </div>
                  </li>
                ))}
              </ul>
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead className="hidden sm:table-cell">Anfitrión</TableHead>
                        ...
                        <Fragment key={o.id}>
                          ...
                          >
                            ...
                            <TableCell>
                              ...
                              <p className="text-[11px] text-muted-foreground">Pedido #{o.id}</p>
                              {open && <OrderExpand order={o} />}
                            </TableCell>
